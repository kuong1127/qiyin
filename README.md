...
栖音是一款适用于 Windows 11 的轻量任务栏音乐控制组件。它把封面、歌曲、播放按钮和爱心放在任务栏附近，方便在工作时控制 QQ 音乐、网易云音乐，以及向 Windows 提供媒体会话的浏览器音视频。
...
1. 打开 [Releases](https://github.com/kuong1127/qiyin/releases/latest)，下载 `Qiyin-Setup-0.2.16-win-x64.exe`。
...
| 点击封面 | 播放器在前台时最小化；最小化、隐藏或被遮挡时恢复到前台 |
...
组件监听歌曲、播放状态和时间轴事件，并结合周期刷新更新画面。多来源同时存在时，暂停后保持当前来源，其他来源新开始播放时可自动切换；右键菜单的“播放来源”可锁定当前运行期间的指定会话，来源关闭后回退。没有媒体会话时，指定播放器可退回窗口标题读取。
...
界面采用位于任务栏上方的顶层小窗口，尽量避免点击操作抢走当前工作的焦点。控制器负责跟踪任务栏和显示器位置、缩放比例、托盘入口以及需要避让的常驻窗口。它没有替换 Windows 任务栏。当前屏幕进入全屏视频或全屏应用时，组件自动隐藏，退出全屏后恢复。
...
长歌词使用连续位移动画展示后半句；短句静止。已取得歌词时，短暂空白保留上一句，长空白显示“间奏”。没有有效进度或没有匹配歌词时显示艺人信息。歌词获取失败不会阻塞播放按钮，短暂失败会有限制地重试。
...
适用于 Windows 11 x64。下载 `Qiyin-Setup-0.2.16-win-x64.exe` 安装；已有用户先退出旧版再覆盖安装，保留原有配置。此版本不支持 Windows 10。

相较上一公开版本 0.2.10：
- 全屏视频或应用覆盖当前屏幕时自动隐藏，退出全屏后恢复。
- 支持 Windows 提供媒体会话的浏览器音视频；暂停后保持当前来源，可在右键菜单的“播放来源”中手动锁定。按钮按来源能力显示，不保证所有网页均可控制。
- 长歌词平滑横向滚动，暂停时停住、跳转和换句时重新对齐；短暂句间保留上一句，长间奏显示“间奏”。
- 进度条内收并使用圆润端点，避免两侧被窗口圆角裁切。
- 封面读取失败后延迟重试；音乐封面可在后台按歌名和歌手匹配补取，保留同一首已加载图片。
- 限制封面解码尺寸和歌词内存缓存，减少重复控件扫描及任务栏查询。长期内存稳定性仍需继续观察，不承诺固定占用上限。

验证：本机 Windows 11 x64，81 项逻辑检查、55 项原生界面检查通过；安装文件与构建文件一致，已有配置保留。实际确认了 Catch in the Dark 封面补取成功。不代表所有播放器版本均已验证。

网易云原生时间轴限制仍保留；没有有效时间轴时不提供同步歌词和进度跳转。安装包未配置代码签名，SHA256.txt 用于核验完整性。

## 历史版本与验证范围

`0.2.10` 增加封面点击切换播放器窗口：前台最小化，后台或隐藏时恢复；恢复后保留最大化状态。QQ 音乐与网易云音乐均已完成本机封面点击检查，核心回归检查 62 项通过。公开发布构建关闭调试符号，移除编译产物中的本机开发目录；发布资料采用独立清单，避免夹带内部验证文件。
